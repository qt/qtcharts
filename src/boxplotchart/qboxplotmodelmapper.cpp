...
/*!
    Sets which row of the model contains the \a first values of the QBoxSets in the series.
    The default value is 0.
*/
...
void QBoxPlotModelMapperPrivate::modelUpdated(QModelIndex topLeft, QModelIndex bottomRight)
{
    ...
    for (int row = topLeft.row(); row <= bottomRight.row(); row++) {
        for (int column = topLeft.column(); column <= bottomRight.column(); column++) {
            ...
            if (box) {
                if (m_orientation == Qt::Vertical)
                    box->replace(row - m_first, m_model->data(index).toReal());
                else
                    box->replace(column - m_first, m_model->data(index).toReal());
            }
        }
    }
...
void QBoxPlotModelMapperPrivate::modelHeaderDataUpdated(Qt::Orientation orientation, int first, int last)
{
    ...
    qDebug() << "ALERT: QBoxPlotModelMapperPrivate::modelHeaderDataUpdated implement";
}
...
void QBoxPlotModelMapperPrivate::boxSetsAdded(QList<QBoxSet *> sets)
{
    ...
    for (int i = firstIndex + m_firstBoxSetSection; i < firstIndex + m_firstBoxSetSection + sets.count(); i++) {
        //m_model->setHeaderData(i, m_orientation == Qt::Vertical ? Qt::Horizontal : Qt::Vertical, sets.at(i - firstIndex - m_firstBoxSetSection)->label());
