...
{
    QStringList labels = createLabels(m_ticks,m_min,m_max);
...
void AxisItem::setLayout(const QVector<qreal>& layout)
{
    ...
    Q_ASSERT(layout.size() == m_thicksList.size());
...
void AxisItem::handleTicksCountChanged(int ticks)
{
    m_ticks=ticks;
